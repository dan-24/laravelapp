  ...
  <div>
        ...
        <v-card>
          ...
          <v-card-text>
            <v-container grid-list-md>
              <v-layout wrap>
                <v-flex xs12>
                  <v-text-field v-model="editedItem.nama_kategori" label="Nama Kategori"></v-text-field>
                </v-flex>
                <v-flex xs12>
    ...
    <v-data-table :headers="headers" :items="datatable" class="elevation-1">
      <template v-slot:items="props">
        <td>{{ props.item.nama_kategori }}</td>
  ...
  methods: {
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        axios
          .put("/api/kategori/" + this.editedItem.id, this.editedItem)
          .then(response => {
            console.log(response);
          });
      } else {
        axios
          .post("/api/kategori/", this.editedItem)
          .then(response => {
